...
//         src/data/roadtripRentalPaths.ts
...
const RENTAL_PATHS_OUTPUT = join(
  __dirname,
  "../src/data/roadtripRentalPaths.ts",
);
...
async function main() {
  // ── Main driving route ────────────────────────────────────────────────────
  const stops = roadtrip.stops.filter(
    (s) => s.kind !== "flight" && s.kind !== "rental",
  );
...
`;
  ...
  // ── Rental driving paths ──────────────────────────────────────────────────
  console.log("\n── Rental paths ──");
  const rentalStops = roadtrip.stops.filter(
    (s) => s.kind === "rental" && s.flightFrom,
  );
  const rentalEntries: string[] = [];

  for (const rental of rentalStops) {
    const fromCoord = rental.flightFrom!;
    const toCoord: [number, number] = [rental.lng, rental.lat];
    let segment: [number, number][] | null = null;
    try {
      segment = await fetchSegment([fromCoord, toCoord]);
    } catch (err) {
      console.warn(
        `  [WARN] Rental ${rental.id}: fetch error: ${err}. Using straight line.`,
      );
    }

    if (!segment) {
      console.warn(
        `  [WARN] Rental ${rental.id}: OSRM returned no route. Using straight line.`,
      );
      segment = [fromCoord, toCoord];
    } else {
      console.log(`  [OK]   Rental ${rental.id}: ${segment.length} points`);
    }

    const lines = segment
      .map(([lng, lat]) => `    [${lng}, ${lat}]`)
      .join(",\n");
    rentalEntries.push(`  "${rental.id}": [\n${lines},\n  ]`);

    await sleep(THROTTLE_MS);
  }

  const rentalOutput = `// Auto-generated by scripts/build-roadtrip-route.ts. Do not edit manually.

export const rentalPaths: Record<string, [number, number][]> = {
${rentalEntries.join(",\n")},
};
`;

  writeFileSync(RENTAL_PATHS_OUTPUT, rentalOutput, "utf8");
  console.log(`\nWrote ${RENTAL_PATHS_OUTPUT}`);
}
